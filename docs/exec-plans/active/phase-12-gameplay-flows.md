...
- host-owned start flow that builds the final roster from the latest lobby state plus any observed concurrent joiners without requiring explicit "Record Join" as the primary product path
...
- Stage 12.1 landed as a product lobby shell rather than another expansion of the debug view. The lobby now auto-sends join actions, advances fresh joins through canonical lobby state, derives pending participants from the latest lobby roster plus observed concurrent joiners, and keeps host start as the single canonical transition into setup.
...
- Stage 12.5 moves trade into a compact modal and shell-visible status path: current players compose player trade explicitly, maritime trade stays as a legal quick-trade list, non-current players can send accept intents, and the live offer remains legible in the shell instead of depending on hidden shortcut rows.
- 2026-04-18: the rules-faithful split for resource flows is now explicit. Discard no longer relies on generated default payloads, player trade stays fully composed, and maritime trade uses a quick-trade list because the engine already enumerates the complete legal option set.
