...
| UCR-002 | mechanical | DESIGN.md approved component vocabulary and 2026-08-10 user legacy-free audit request | No retired lower-tray, shelf, duplicate rules engine, obsolete production layout branch, or dead compiled legacy renderer/component remains; DEBUG fixtures reference production components rather than duplicate UI | route/component inventory, identifier audit, compiler/build, deleted-symbol source guard, and independent code audit | report:terra-two-auditor-legacy-scan-2026-08-10 | blocked | Production routing is canonical, but the fresh audit found an unreachable legacy Trade renderer plus three unreferenced legacy component files still compiled into the extension; delete them and add the source guard before passing this constraint |
| UCR-003 | observable | user request and UI flows | Every production control has a working open/close/back/cancel/confirm/selection path, including full Trade, Tutorial, robber, Games, lifecycle, and terminal UX | locked focused XCUITest journeys on a freshly installed build and a freshly built matching UI-test bundle | report:stale-uitest-runner-diagnosis-2026-08-10 | blocked | The cold-restart result proves the extension launched, then failed because `test-without-building` used an old UI-test runner that still expected `Invitation Card`; rebuild and pin the runner and installed app from one DerivedData root before rerunning observable proof |
...
- Restarted simulator attempt: clean build-for-testing passed; `/tmp/uls-final-ui-20260810/restarted-lobby-runtime.xcresult` is a valid failed result. It proves the extension launched and identifies a stale UI-test binary expecting `Invitation Card`, not a simulator or production-app crash.
