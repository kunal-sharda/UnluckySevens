...
| --- | --- | --- | --- | --- | --- | --- | --- |
| TS-001 | observable | user plan | Each comparison round shows Default Hand, Build spread, and revealed Bank using real components and fixture data | DEBUG comparison capture and artifact inspection | report:Round 1 and Round 2 direct simulator stills in ignored `docs/design/workbench/physical-props-round{1,2}-{hand,build,bank}.png` | satisfied | Both rounds use the existing `turn-after-roll` fixture and real code-native components |
| TS-002 | judgment | user plan | User explicitly approves the post-roll physical turn-surface metaphor before nested productionization | user verdict recorded as a dated decision | report:pending-user-approval | pending | Hard approval gate |
| TS-003 | mechanical | architecture and product owners | Available objects and choices derive from Core/query and presentation outputs rather than duplicated UI rules | focused tests and code inspection after implementation | command:pending | pending | Required after approval |
| TS-004 | observable | UI flows and Design | Approved nested states preserve board dominance, stable geometry, clear object ownership, accessibility, and legal interaction behavior | focused UX Lab/XCUITest journey plus direct still inspection | report:pending | pending | Required after productionization |
| TS-005 | judgment | user-approved comparison | Production result faithfully implements the approved composition | final user evidence check and final product/UX review | report:pending | pending | Required after productionization |
| TS-006 | observable | Round 13 user direction | Physical rails preserve their hierarchy on both compact and tall supported phone hosts: labels sit below props, public objects retain breathing room above the board, and the canonical island—not its container—is centered on the full display | pure layout tests plus installed captures on two phone heights | report:Round 13 tall Default, compact Default, and tall Build captures | satisfied | Tall island midpoint is exactly `1311px`; compact midpoint is within `0.5px` of `1266px`; both retain unclipped public/action rails |
<!-- verification-contract:end -->

<!-- fresh-review:start -->
| Reviewer | Required | Verdict | Evidence |
| --- | --- | --- | --- |
| constraint-auditor | yes | pending | report:pending-after-productionization |
...
| product-ux | yes | pending | report:final-only-after-productionization |
<!-- fresh-review:end -->
...
- [ ] Approved direction productionized and finally verified.
...
Rounds 1–25 are implemented only behind the DEBUG `physicalProps` UX Lab comparison style; production routing remains unchanged. Round 15 centers Bank and Dev Cards as one 28-point-gap group, gives both an eight-point card-to-label layout gap, returns the visible teal board frame to the shell content width without resizing the live SpriteKit host, and optically shifts the flag foot right. Rounds 16–17 refine Bank spacing, terrain-visible tokens, and canonical City geometry. Round 18 adds pip dice and solid structures. Round 19 thins roads, enlarges structures, extends the ocean only below, and removes the inset/host seam so the rounded teal field reads flat. Round 20 removes the misleading ocean-thread overlay and replaces rate-label port badges with physical port markers. Round 21 reuses the approved Trade ship asset for every generic port and shortens/quiets the offshore pier lines without pulling the ports onto the island. Round 22 adds a single subtle center-to-edge tonal fill to the same rounded ocean node. Round 23 compares three deterministic texture-backed ocean fields and records shoreline halo as the selected direction. Round 24 strengthens piece hierarchy with thinner roads and larger settlements/cities on both the live board and Build spread without changing anchors or geometry. Round 25 adds a darker owning-color edge to live structures for terrain separation without introducing a neutral outline. The installed Round 25 Halo capture test and all thirteen focused board-scene tests pass. No reviewer was launched during the checkpoint. Compact revalidation, specialist reviews, nested-state productionization, and the completion gate remain intentionally deferred until user approval.
