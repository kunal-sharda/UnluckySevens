...
- Round 30: requested 2026-07-16; add breathing room to the Dev Cards name tile, optically strengthen the fine sheep mark on tabletop cards and build costs, and overscan the SpriteKit ocean so camera panning cannot reveal the finite backdrop seam
...
- 2026-07-13: The smoky-indigo pass was rejected as too purple. Round 5 uses a desaturated blue-gray card family, the user-supplied Noun Project factory and ship vectors, and one bold warm-white H/M/L treatment across all six public piles. Source attribution is retained in `docs/design/README.md`.
...
- 2026-07-16: Round 30 keeps the approved public geometry but widens the `Dev Cards` wooden name tile for Bank-like side padding. The sheep stamp receives a one-device-pixel optical expansion only in small SwiftUI tabletop contexts. The ocean backdrop overscans the maximum camera slack, preventing its rounded texture edge from appearing as horizontal or vertical lines during panning while the visible SwiftUI board frame retains the approved rounded corners.
- 2026-07-16: Round 30 focused verification passed: 37 board, camera, responsive-layout, and roll-phase tests plus the installed `testSettleTurnBankForDirectStill` journey completed with zero failures. Direct connected-device judgment remains the checkpoint for optical sheep weight and dice travel.
