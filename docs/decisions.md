...
- Player Record remains backed by device-local observations, but it does not show a device-storage disclosure. That scope is an implementation limitation rather than useful screen content; the record stays read-only and never offers recovery or publication actions.
- Settings presents Skip animations directly, then current-game facts, the Rules destination, and an accessible Privacy Policy link without separate Experience or Help headings or explanatory animation copy.
