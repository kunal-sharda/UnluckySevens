...
- Confidence: Medium.
- This plan is reconstructed primarily from `CHANGELOG.md`, the current repo layout, and the surviving early debug transport code. Commit-level evidence for the stage 0 work is incomplete.
...
- Evidence: `CHANGELOG.md` entries for stage 0.A covering bootstrap scaffolding, `scripts/gen.sh`, `scripts/clean.sh`, `.gitignore`, `README.md`, and `Makefile`.
...
- Evidence: `CHANGELOG.md` stage 0.B entries and the current descendants in `Project.swift`, `Workspace.swift`, `Tuist.swift`, `App/`, and `Packages/`.
...
- Evidence: `CHANGELOG.md` stage 0.C entries and current `ULS_Transport/Sources/ULS_Transport/DebugPayload.swift` plus `MessagesExtension/Sources/MessagesViewController.swift`.
...
- Evidence: `CHANGELOG.md` stage 0.C.1 entries.
...
- Evidence: `CHANGELOG.md` stage 0.C.2 entries and current `DebugPayload` fields/tests.
...
- Evidence is strongest in `CHANGELOG.md` and the surviving transport tests, not in preserved active plans.
