...
For Messages entry-routing changes, run the focused `scripts/ui-screenshot-catalog-launch-routing.txt` catalog through `scripts/run-ui-screenshot-catalog.sh`. It seeds saved games, relaunches Messages, proves the fresh lobby has no active canonical game, and then proves those records remain available through Your Games. Automatic expansion is host-controlled and must not be asserted by this routing lane; final release proof records drawer, transcript-bubble, and Your Games presentation style and bounds on real devices.
