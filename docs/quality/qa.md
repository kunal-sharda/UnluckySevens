...
For Messages entry-routing changes, run the focused `scripts/ui-screenshot-catalog-launch-routing.txt` catalog through `scripts/run-ui-screenshot-catalog.sh`. It seeds local records, relaunches Messages, proves the fresh lobby has no active canonical game, and then proves those records remain read-only in Player Record. Automatic expansion is host-controlled and must not be asserted by this routing lane; final release proof records drawer, transcript-bubble, and Player Record presentation style and bounds on real devices.

For Messages resize or gesture changes, begin with Apple’s current Messages framework contract: on iOS 17 and later, Messages owns interactive vertical resizing and mediates conflicts with custom gestures. Use a physical-device matrix for lobby controls, SpriteKit taps/pan/pinch, scrollable utilities, and host resize/collapse before adding or changing a controller-wide recognizer. Do not lock a top-only resize promise unless the platform behavior is proven enforceable without breaking local input.
...
Before any canonical Messages screenshot lane, shut down every simulator, boot only the target device, uninstall `me.ksharda.games.unluckysevens`, install the freshly built `UnluckySevensApp.app`, and restart Messages before `test-without-building`. Installing over a running prior host is not valid evidence because Messages and XCTest can retain extension or accessibility state. The test action refreshes the runner but does not guarantee replacement of the installed host app; record and compare the built and installed `MessagesExtension` executable hashes.
...
Each selection still inherits the cold lifecycle, isolated-process rule, and diagnostic-first initial-plus-ten retry ceiling. For attempt 2 or later, set `UI_SCREENSHOT_ATTEMPT_NUMBER`, `UI_SCREENSHOT_RETRY_DIAGNOSIS`, and `UI_SCREENSHOT_RETRY_CORRECTION`; the runner rejects undocumented or out-of-budget reruns. If an XCTest navigation surface such as Player Record is not observable, inspect the retained failure bundle and fix its state transition before rerunning; do not use repeated taps or a prior passing catalog as fresh proof.
...
UX Lab action authoring must prefer its explicitly loaded fixture over transcript or local-ledger states with the same game ID. This isolation is DEBUG-only; production actions continue from the newest canonical state associated with the selected or bound Messages game. Player Record ledger snapshots remain read-only.
