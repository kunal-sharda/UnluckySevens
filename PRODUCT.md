...
The product brings a full standard-Catan loop to Messages while preserving deterministic rules, canonical state, and hidden-information boundaries. Success means players can invite, join, set up, play turns, trade, build, use development cards, resume interrupted games from their real Messages bubbles, inspect device-local results, leave play without stopping everyone else, and finish by victory, unanimous draw, or host end without leaving the Messages-first flow.
