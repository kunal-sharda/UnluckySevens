...
- Apple-owned transcript presentation is a compact continuation card, not a miniature game screen: it shows the current game identity/status and one clear **Open Game** action using the existing felt, gold, and SF typography vocabulary.
- Fresh app-drawer entry uses a separate compact entrance: the canonical masked-seven robber lands on the top edge of a raised wood-and-gold **Open Lobby** button. The Skip control settles that decorative motion; Reduce Motion and the persisted Skip Animations preference start settled; Open Lobby reveals the fresh invitation lobby even if Messages keeps the host compact.
...
- Gameplay top bars keep only Settings, status, and Players/Game Information. Player Record is linked from Game Information so history navigation does not disturb the board-first top-bar balance.
- Lobby identity bars keep one compact chart affordance that opens Player Record directly. Below the table, explicit Game Settings and Tutorial rows share text-only hierarchy and chevrons so both destinations read as navigation without crowding the brand row.
- Players/Game Information opens as a centered translucent felt panel with the Trade composer's restrained focus veil, gold keyline, and fixed frame. The live board, water frame, and turn-object rail remain mounted at the same size and position behind it. The roster stays visible; a chart action opens the read-only Player Record, while an ellipsis exposes only lifecycle actions for the currently bubble-bound game.
...
- Supported production gameplay states use one Physical Props shell. Retired lower-tray, shelf, alternate-header, embedded board-rack, and shell-level resize-snapshot implementations are removed rather than retained as hidden fallbacks.
...
- Tutorial coach marks sit beside anchored targets on the production game shell. Guidance uses one compact cream-card treatment, keeps the complete approved copy visible, and avoids anchored controls through collision-aware placement. Standard sizes use at most three numbered bubbles; accessibility sizes replace collision-prone bubbles with a numbered ordered guide. The one-time split veil explains invisible left/back and right/next navigation.
...
- Opening or replacing an action-well surface must not resize, shift, freeze, or remount the live board, public rail, or top status. Board-owned resize continuity may preserve the rendered island internally; the surrounding shell must never substitute a parallel UI composition.
- Messages hosts fit the board and ocean as one canonical-aspect viewport using a single uniform scale derived from the live content region. The island and water never stretch independently. Temporary Players, Games, Trade, Build, and forced-flow surfaces may overlay that mounted tabletop, but must never trigger a new board fit or change its occupied space.
- The bottom tabletop region keeps at least 12 points of total clearance from the Messages host edge. Existing safe-area clearance counts toward that minimum, so zero-inset iPad hosts gain only the missing space and inset iPhones do not gain duplicate padding. Controls and typography retain their established sizes; constrained space comes from flexible board/action regions.
- Number tokens, roads, structures, ports, robber, and legal highlights remain separate live board layers; production terrain art uses the canonical `BoardStamps` assets, with `BoardMiniStamps` reserved for compact renderers.
- Icon-first controls require stable accessibility labels and identifiers.
- Player Record uses one centered, scrollable surface: a paper Catan score sheet for overall win/loss/draw, win rate, fastest win, and current streak; a felt Table Honors row built from the shipped road, Knight-card, dice, and robber vocabulary; then a paper ledger for games whose canonical roster exactly matches the current conversation's players. The ledger uses player-color pieces for standings and remains non-interactive. Its storage remains device-local, but the surface does not add storage-scope copy. Resignation and host end remain in current-game Game Information and use the same centered felt confirmation, gold keyline, and focus veil as Players and Trade.
...
The current production source of truth is the code and assets, especially `GameTheme`, `GameBoardPalette`, and `MessagesExtension/Resources/Assets.xcassets/BoardStamps/`. The six default terrain stamps provide one canonical live-board texture per resource; `BoardMiniStamps` are derived compact variants. SpriteKit owns live gameplay layers and SwiftUI owns the surrounding shell.
