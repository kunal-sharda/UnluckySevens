...
`UnluckySevens.xcodeproj` and `UnluckySevens.xcworkspace` are generated by `./scripts/gen.sh`, may exist locally, and must remain untracked. Tuist manifests are the source of truth. Use `./scripts/gen.sh` for headless generation and `./scripts/gen-local.sh` for local Xcode-launch convenience.

## Quick Start

```bash
bash ./scripts/gen-local.sh
```

Run the `MessagesExtension` scheme, open Messages in Simulator, and launch Unlucky Sevens from the app drawer.

For CI or any headless/local generate-only flow, run:

```bash
bash ./scripts/gen.sh
```

If you want the local helper without opening Xcode, or want a clean pass first, run:

```bash
bash ./scripts/gen-local.sh --no-open
