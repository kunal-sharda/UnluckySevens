...
`UnluckySevens.xcodeproj` and `UnluckySevens.xcworkspace` are generated by `./scripts/gen.sh`, may exist locally, and must remain untracked. Tuist manifests are the source of truth. Use `./scripts/gen.sh` for normal regeneration and `./scripts/gen-local.sh` only when you want extra local helper behavior.
